    ...
    function create_payment_with_paypal()
    {
        ...
        // GET ORDERS REFERENCE ID //
			...
			$orderArray = array(
				...
				'cust_id' => $custid,
				'ord_total_amounts' =>$this->input->post('item_price'),
				);
			//  echo "<pre>";
			// print_r($orderArray);die;
			$ordid=$this->paypal->SaveOrderInformation($orderArray,$this->table_orders);
			if($ordid){
				$orderProducts = array();
				//$total_qty=0;
				...
				$check = $this->paypal->SaveOrderProduct($orderProducts,$this->table_order_products);
				
				
				if($check)
				{
				    $this->session->set_flashdata('orderid',$orderReferenceID);
    ...
    public function getPaymentStatus()
    {
       $ordid = $this->session->flashdata('orderid');
        ...
        if ($result->getState() == 'approved') {
			...
			$updatestatus = $this->paypal->UpdateOrderInformation($ordid,$orderUpdate,$this->table_orders);
            $this->paypal->create($ordid,$this->customer->usr_id,$Total,$Subtotal,$Tax,$PaymentMethod,$PayerStatus,$PayerMail,$saleId,$CreateTime,$UpdateTime,$State);
            $this->session->set_flashdata('success_msg','Payment success');
            redirect('thankyou/success');
        }
        $this->session->set_flashdata('success_msg','Payment failed');
        redirect('thankyou/cancel');
    }
