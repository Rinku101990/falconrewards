...
class Home_model extends MY_Model{
	...
	function coming_product_list($table)
	{
	...
	function winner_list($winid,$table)
	{
   	...
   	function coming_product($pid,$table){
		$this->db->order_by('id',"asc");
		$this->db->where('status','1');	
	 ...
	 function single_coming_product($pid,$table){
		$this->db->order_by('id',"asc");
		//$this->db->where('status','1');	
		$this->db->where('id',$pid);	
		$query=$this->db->get($table);
...
public function Product_Search($fld_value,$table)
	{
		...
		$this->db->join('tbl_reward rwd','camp.reward=rwd.r_id','LEFT');
		$this->db->order_by('pro.p_name',"ASC");
		$this->db->like('pro.p_name', $fld_value);
	    $this->db->where('camp.status', '0');
		$query=$this->db->get($table.' camp');
		//echo $this->db->last_query();
		if($query->num_rows() != 0) return $query->result();
...
function categories_product_list($fld_val,$table){
	...
	function product_detail($fld_val,$table){
		$date=date('Y-m-d');
		$this->db->select('cate.cate_slug,scate.scate_slug,child.child_slug,cate.cate_name,scate.scate_name,child.child_name,tp.p_id,tp.p_vnd_id,tp.p_reference_no,tp.p_name,tp.p_model,tp.p_selling_price,tp.p_lenght,tp.p_width,tp.p_height,tp.p_weight,tp.p_barcode,tp.p_short_description,tp.p_description,pl.p_warranty_policy,pl.p_return_policy,ut.ut_dime_name,ut2.ut_weight_name,pl.p_option_group,pl.option_grop,pl.option_grop_add,pl.option_grop,int.int_stock,int.int_min_purchase_qty,sp.sp_special_price,sp.sp_end_date,sp.sp_start_date,pimg.pg_image,brd.brd_name');	
		$this->db->order_by('tp.p_id',"DESC");
		$this->db->join('tbl_category cate','cate.cate_id=tp.p_cate','LEFT');		
		$this->db->join('tbl_sub_category scate','scate.scate_id=tp.p_scate','LEFT');
		$this->db->join('tbl_child_category child','child.child_id=tp.p_child','LEFT');
...
function make_query($page,$brand,$color,$size,$price,$grade,$condition,$category,$sub_category,$child_category,$hot,$featured,$trending,$supplier,$newest_first)
 { 
  $date=date('Y-m-d');   
  $query = "SELECT `tp`.`p_id`, `tp`.`p_cate`, `tp`.`p_scate`, `tp`.`p_child`, `tp`.`p_reference_no`, `tp`.`p_name`, `tp`.`p_model`, `tp`.`p_selling_price`, `tp`.`p_short_description`, `sp`.`sp_end_date`,`cate`.`cate_slug`, `scate`.`scate_slug`, `child`.`child_slug`, `sp`.`sp_special_price`,`sp`.`sp_end_date`,`sp`.`sp_start_date`, `pimg`.`pg_image` FROM `tbl_product` `tp` LEFT JOIN `tbl_special_price` `sp` ON `sp`.`sp_pid`=`tp`.`p_id` LEFT JOIN `tbl_product_img` `pimg` ON `pimg`.`pg_pid`=`tp`.`p_id` LEFT JOIN `tbl_category` `cate` ON `cate`.`cate_id`=`tp`.`p_cate` LEFT JOIN `tbl_sub_category` `scate` ON `scate`.`scate_id`=`tp`.`p_scate` LEFT JOIN `tbl_child_category` `child` ON `child`.`child_id`=`tp`.`p_child` LEFT JOIN `tbl_product_link` `pl` ON `pl`.`p_id`=`tp`.`p_id` LEFT JOIN `tbl_inventory` `int` ON `int`.`int_pid`=`tp`.`p_id` LEFT JOIN `tbl_vendor` `vnd` ON `vnd`.`vnd_category`=`tp`.`p_vnd_id` WHERE `tp`.`p_status` = '1' AND `tp`.`p_approval_status` = '0' 
    ";  
...
function product_data($page,$brand,$color,$size,$price,$grade,$condition,$category,$sub_category,$child_category,$hot,$featured,$trending,$supplier,$newest_first)
 {
 	$query = $this->make_query($page,$brand,$color,$size,$price,$grade,$condition,$category,$sub_category,$child_category,$hot,$featured,$trending,$supplier,$newest_first);
