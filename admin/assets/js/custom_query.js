$(document).ready(function(){
    ...
    $('input[type="file"]').change(function(event) {
        ...
        // var validExtensions = ["mp4","webm","avi","flv","mov"]
        // var file = $(this).val().split('.').pop();
        // if (validExtensions.indexOf(file) == -1) {
        //     alert("Only formats are allowed : "+validExtensions.join(', '));
        // }
        
    	if(exactSize>customSize) {
    	    $(".AlertMessageFile").css("color","#FF0000");
    	    $(".AlertMessageFile").html(" (Size:"+exactSize+")");
    	    $("#drwvideo").css("color","#FF0000");
    		$("#drwvideo").html("Allowed file size is less than 5MB");
    		$(".drwvideo").css("border-color","#FF0000");
    		$('.btn2').prop('disabled', true); // disabled sumbit button
    		return false;
    	}else{
    	    $(".AlertMessageFile").css("color","#65c347");
    	    $(".AlertMessageFile").html(" (Size:"+exactSize+")");
    	    $("#drwvideo").css("color","#65c347");
    		$("#drwvideo").html("Allowed file size is less than 5MB");
    		$(".drwvideo").css("border-color","#aab3ce");
    		$('.btn2').prop('disabled', false); // enable submit button
    		return true;
    	}
    });

    // GET COUNTRY CODE //
//     $(".drw_category").on("change", function(){
// 		let url = $('#site_url').val();
// 		let cateId = $(this).val();
//         $.ajax({
//             method:"POST",
//             url:url+"draws/get_product_list_by_category",
//             data:{cateId:cateId},
//             dataType:"json",
//             success:function(data){
//                 $("#drw_products").html('<option value='+data.products.pro_id+'>'+data.products.p_name+'</option>');
//             }
//         });
//     });
...
function validate() {
	$("#drwvideo").html("");
	$(".drwvideo").css("border-color","#F0F0F0");
	var file_size = $('#drw_video')[0].files[0].size;
	if(file_size>5120) {
	    $(".AlertMessageFile").css("color","#FF0000");
	    $(".AlertMessageFile").html(" (Size:"+file_size+")");
	    $("#drwvideo").css("color","#FF0000");
		$("#drwvideo").html("Allowed file size is less than 5MB");
		$(".drwvideo").css("border-color","#FF0000");
		return false;
