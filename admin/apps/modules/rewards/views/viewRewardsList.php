   ...
   <div class="section">
      ...
      <div class="page-header">
         <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active" aria-current="page">Manage Reward</li>
         </ol>
         <div class="mt-3 mt-lg-0">
            <div class="d-flex align-items-center flex-wrap text-nowrap"> 
               <button type="button" onclick="history.go(-1)" class="btn btn-secondary btn-icon-text mr-2 mb-2 mb-md-0"> <i class="fa fa-arrow-left"></i> Go Back  </button>
               <a href="<?=base_url('rewards/reward_add');?>">
               <button type="button" class="btn btn-primary btn-icon-text mb-2 mb-md-0"> <i class="fe fe-plus "></i> Add Reward </button>
               </a>
            </div>
         </div>
      </div>
                        ...
                        <div class="row">
                           <div class="col-sm-12">
                              <table id="example" class="table table-striped table-bordered dataTable no-footer" role="grid" aria-describedby="example_info">
                                 ...
                                 <tbody>
                                    <?php if(!empty($reward)){
                                       $i=1;  foreach($reward AS $list){?>
                                    <tr role="row" >
                                       <td class="sorting_1" align="center"><?=$i;$i++;?>.</td>
                                       <td> <img src="<?=base_url('uploads/reward/').$list->image;?>" style="width:70px; height: 50px"></td>
                                       <td > <?=$list->r_title;?></td>
                                        <td > <?=category($list->r_category);?></td>
                                     <td align="center">
                                       <a href="javascript:void(0);"onclick="javascript:showMyModalSetTitle('Change Status','rewards/reward_status/',<?=$list->r_id;?>,<?php if($list->r_status=='0'){echo'1';}else{echo'0';}?>)">
                                          <span class="badge badge-<?php if($list->r_status=='1'){echo'danger';}else{echo'success';}?>-light badge-md"> <?php if($list->r_status=='1'){echo'In-Active';}else{echo'Active';}?></span></a>
                                       </td>
                                    <td > <?=date('d M, Y h:i:s',strtotime($list->r_created));?></td>
                                       <td align="center"> 
                                          <a href="<?=base_url('rewards/reward-edit/').encode($list->r_id);?>" class="btn btn-info btn-sm mb-2 mb-xl-0 text-white" data-toggle="tooltip" data-original-title="Edit"><i class="fa fa-edit"></i></a>
                                        
                                          <a href="javascript:void(0);" onclick="javascript:showMyModalSetTitle('Delete','rewards/reward_delete/',<?=$list->r_id;?>,'')" class="btn btn-danger btn-sm mb-2 mb-xl-0 text-white" data-toggle="tooltip" data-original-title="Delete"><i class="fa fa-trash"></i></a>
                                       </td>
                                    </tr>
                                    <?php }}?>
                                 </tbody>
