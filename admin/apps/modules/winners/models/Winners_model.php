<?php 
class Winners_model extends MY_Model{
	...
	public function getWinnerslist($winid,$table)
	{
		$this->db->select('win.*,usr.usr_fname,usr.usr_lname,tckt.ticket_no,tckt.price,drw.drw_title');
		$this->db->join('tbl_users usr','usr.usr_id=win.usr_id','LEFT');
		$this->db->join('tbl_ticket tckt','tckt.id=win.ticket_id','LEFT');
		$this->db->join('tbl_draws drw','drw.drw_id=win.draw_id','LEFT');
		$this->db->order_by('win.'.$winid,'RANDOM');
	...
	{	
	    $this->db->select('usr_id,usr_fname,usr_lname');
	...
	{
        $this->db->where('ticket_id',$drwname);
        $this->db->limit(1);		
