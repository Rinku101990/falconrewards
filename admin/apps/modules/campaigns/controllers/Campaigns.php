...
class Campaigns extends MY_Controller {
	...
	public function __construct() {
        ...
        $this->load->model("Campaigns_model",'Product');
       /* ========FOR MASTER TABEL=========== */ 			
    ...
    {  
     	...
     	if(empty($checkName)){  
	        ...
	        if($result){
			   ...
			   redirect('campaigns/campaign_add'); 
		   }else{
	    ...
	    if($RequestMethod == "POST") { 	
	        ...
	        if(empty($_FILES['img']['name'])){
	    	    $image_data=$this->input->post('prev_img');
     	        ...
     	        $path=FCPATH . 'uploads/campaign';
        		$image_name='img';
        		$img1= $path.'/'. $content['campaign']->img;  
        		if (!unlink($img1)) {} else { }
        		$image_data=$this->Product->images_upload($image_name,$path);  
                // Image
            }
	        ...
	        if($result){
			   ...
			   redirect('campaigns/campaign-edit/'.$this->uri->segment(3)); 
		   }else{
			   $this->session->set_flashdata('msg',array('message' => "Unable to Campaign .Some error occurred.",'class' => 'danger','type'=>'Oops!','icon'=>'slash'));
			    redirect('campaigns/campaign-edit/'.$this->uri->segment(3));   
		   }
	   ...
	   {	 
	     $content['admin']=admin_profile($this->login->mst_email);	       
	   	...
	   	$status=$this->uri->segment(4); 
	   		$campaign = $this->Product->single_product('id',$id,$this->table_campaign);
		$price=reward($campaign->reward);
	   	//ticket
	   	$data_tk= array('status' =>$status);
	   	// echo"<pre>";
	   	// print_r($price);
	   	// die;
	      //ticket save      
	      $result_ticket = $this->Product->update('price',$price,$data_tk,'tbl_ticket');
	//end 
	      $data= array(
	                   'status' =>$status,
	                   'updated' =>date('Y-m-d H:i:s')
